...
- name: {{ $containerName }}
  ...
  env:
    ...
    - name: CLICKHOUSE_PASSWORD
      valueFrom:
        secretKeyRef:
          name: {{ if and (hasKey $root.Values.runner "clickhouse_password") $root.Values.runner.clickhouse_password }}{{ include "nudgebee-agent.fullname" $root }}-runner-secret{{ else }}{{ $clickhouseSecret }}{{ end }}
          key: {{ if and (hasKey $root.Values.runner "clickhouse_password") $root.Values.runner.clickhouse_password }}CLICKHOUSE_PASSWORD{{ else }}admin-password{{ end }}
    {{- end }}
