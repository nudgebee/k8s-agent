...
| `ELASTICSEARCH_HEADER` | optional | Semicolon-separated `Header: value` pairs sent on every ES request (same format as PROMETHEUS_HEADERS). `ELASTICSEARCH_HEADERS` is accepted as an alias — it is what the chart's `runner.es.headers` emits — and `ELASTICSEARCH_HEADER` wins if both are set |
