...
func (c *Client) Search(ctx context.Context, index, queryType string, query any) (json.RawMessage, error) {
	...
	case "ppl":
		...
		//
		// The case-insensitive check on both "source=" and "search source="
		// mirrors es_client.query_elasticsearch_dsl, which lowercases before
		// testing for either form.
		s = strings.TrimSpace(s)
		lowered := strings.ToLower(s)
		if !strings.Contains(lowered, "source=") && !strings.Contains(lowered, "search source=") {
			if s == "" {
				s = "source=" + index
			} else {
				s = "source=" + index + " | " + s
