...
// TestSearch_PPL_SourceCheckIsCaseInsensitive: legacy lowercases before
// testing for an existing source clause, so "SOURCE=" must not be
// double-prefixed.
func TestSearch_PPL_SourceCheckIsCaseInsensitive(t *testing.T) {
	var gotBody string
	c, srv := newTestClient(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
		gotBody = string(body)
	...
	if strings.Count(strings.ToLower(gotBody), "source=") != 1 {
		t.Errorf("expected exactly one source= clause: %s", gotBody)
	}
}
